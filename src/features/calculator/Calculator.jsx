...
import { matchaCostPerCup, milkCostPerCup, cogsForDrink, profit, marginPct, marginWord, tallyTotals } from "@/features/calculator/cost";
...
export default function Calculator({ matchaOptions, usingAllPowders, milkOptions, drinks, savedDrinks, ingredients, costs, srp, priceOverrides }) {
  ...
  const [mLabel, setMLabel] = useState(() => {
    let best = matchaOptions[0], bd = Infinity;
    for (const o of matchaOptions) { const d = Math.abs(o.g - 20); if (d < bd) { bd = d; best = o; } }
    return best.l;
  });
  ...
  useEffect(() => { if (selIndex === -1) setMLabel(matchaOptions[0].l); }, [selIndex, matchaOptions]);
    ...
    <>
      ...
      <section className={SECTION}>
        ...
        <div className="flex gap-[18px] flex-wrap max-md:gap-3">
          ...
          >
            {milkOptions.map((o, i) => (<option key={i} value={i}>{o.l}</option>))}
          </SelectField>
        </div>
        {usingAllPowders && (
          <div className="mt-1 font-mono text-[.56rem] tracking-[.02em] text-brown-soft">Showing every powder — <a href="/powders" className="text-clay underline underline-offset-2">♥ heart your picks</a> to narrow the matcha list.</div>
        )}
      ...
      {saved.length === 0 ? (
        <div className="paper-card p-6 text-center">
          <div className="font-doodle font-bold text-[1.2rem] text-forest mb-1">No drinks costed yet</div>
          <p className="text-[.86rem] text-olive-soft">♥ heart the drinks you sell on the <a href="/drinks" className="text-clay underline underline-offset-2 font-semibold">Drinks page</a> and they’ll show up here with ingredient prices, COGS, margin &amp; totals.</p>
        </div>
      ) : (
        <>
          {/* ② Ingredient prices — above the drinks, only what the hearted drinks use */}
          <section className={SECTION}>
            <SectionTitle badge="②" title="Ingredient Prices" meta="reference baked in · override to adjust (shared)" />
          ...
          <section className={SECTION}>
            <SectionTitle badge="③" title="List of Drinks" meta={`${saved.length} hearted · set cups per drink`} />
                ...
                return (
                  <article key={d.name} className="relative bg-cream-deep border-[2.2px] border-brown rounded-card shadow-hard-brown p-[17px] flex flex-col gap-[11px]">
                    <div className="flex gap-[11px] items-center">
                      ...
                      <div className="flex-1 min-w-0"><h3 className="font-doodle font-bold text-[1.12rem] text-brown leading-tight">{d.name}</h3><span className="font-mono text-[.58rem] tracking-[.04em] text-brown-soft">{d.note}</span></div>
                      <label className="text-center font-mono text-[.5rem] tracking-[.1em] uppercase text-brown-soft">cups<NumberField variant="underline" aria-label={"Cups of " + d.name} inputMode="numeric" inputClassName="!w-[58px] text-center !text-forest" min="0" step="5" value={n} onChange={(e) => setCups((c) => ({ ...c, [d.name]: Math.max(0, +e.target.value || 0) }))} /></label>
                    </div>
                    <div className="grid grid-cols-3 gap-[7px]">
                      <div className="cost-cell"><span className="block font-mono text-[.52rem] tracking-[.1em] uppercase text-brown-soft">COGS</span><span className="block font-mono font-medium text-[1.05rem] text-forest mt-[3px]">{peso(cogs)}</span></div>
                      <div className="cost-cell"><span className="block font-mono text-[.52rem] tracking-[.1em] uppercase text-brown-soft">SRP ✎</span><NumberField variant="underline" inputMode="numeric" min="0" step="5" value={s} aria-label={"Selling price for " + d.name} onChange={(e) => editSrp(d.name, +e.target.value || 0)} onBlur={() => commitSrp(d.name)} /></div>
                      <div className="cost-cell cost-cell--hl"><span className="block font-mono text-[.52rem] tracking-[.1em] uppercase text-onforest-mut">Profit</span><span className="block font-mono font-medium text-[1.05rem] text-cream-light mt-[3px]">{peso(p)}</span></div>
                    </div>
                    <div className="flex justify-between font-mono text-[.56rem] tracking-[.06em] uppercase text-brown-soft"><span>gross margin · <b className="text-clay">{mar}%</b></span><span>{marginWord(mar)} · ×{n} = {peso(p * n)}</span></div>
                    ...
                    <div className="font-mono text-[.56rem] leading-relaxed tracking-[.01em] text-brown bg-cream-card border-[1.5px] border-dashed border-brown-soft rounded-[11px] px-[11px] py-[9px]">🧮 {parts.join(" + ")} = <b className="text-forest">{peso(cogs)}</b></div>
                  </article>
                );
              })}
            </div>
          ...
          <section>
            <SectionTitle badge="④" title="Totals" meta={`${tot.cups} cups across ${saved.length} drinks`} />
            ...
            <div className="grid grid-cols-3 gap-3 max-md:grid-cols-1">
              <div><div className="font-mono text-[.54rem] tracking-[.12em] uppercase text-onforest-mut">Total COGS</div><div className="font-display font-bold text-2xl">{peso(tot.cogs)}</div></div>
              <div><div className="font-mono text-[.54rem] tracking-[.12em] uppercase text-onforest-mut">Total revenue</div><div className="font-display font-bold text-2xl">{peso(tot.revenue)}</div></div>
              <div><div className="font-mono text-[.54rem] tracking-[.12em] uppercase text-onforest-mut">Total profit</div><div className="font-display font-bold text-2xl text-matcha-bright">{peso(tot.profit)}</div></div>
            </div>
            ...
            {Object.keys(tot.addonByName).length > 0 && (
              <div className="mt-2.5 bg-cream-light/10 border border-onforest-mut/30 rounded-[12px] p-3">
                <div className="font-mono text-[.52rem] tracking-[.12em] uppercase text-onforest-mut mb-2">🍓 add-ons breakdown · {peso(tot.addons)} total</div>
            ...
            <div className="mt-2.5 font-mono text-[.58rem] tracking-[.02em] text-onforest-soft">COGS = matcha {peso(tot.matcha)} + milk {peso(tot.milk)} + add-ons {peso(tot.addons)} + packaging {peso(tot.packaging)} + additional {peso(tot.additional)} = <b className="text-cream-light">{peso(tot.cogs)}</b>. Ingredient cost only — excludes booth fee, labour &amp; spoilage.</div>
            </div>
          </section>
        </>
      )}
...
function Brk({ label, v }) {
  return (
    <div className="bg-cream-light/10 border border-onforest-mut/30 rounded-[10px] px-2.5 py-1.5">
      <div className="font-mono text-[.5rem] tracking-[.08em] uppercase text-onforest-mut">{label}</div>
...
function PriceRow({ label, unit, refVal, okey, ov, editOv, commitOv, link }) {
  ...
  return (
    <div className="ing-tile">
      <div className="font-mono text-[.56rem] tracking-[.04em] uppercase text-olive truncate">{label}</div>
      <div className="flex items-center gap-2 mt-0.5">
        <span className="font-mono text-[.62rem] text-brown-soft whitespace-nowrap">ref ₱{refVal}{unit}</span>
        <NumberField
          variant="underline"
      ...
      {link && (
        <a href={link} target="_blank" rel="noopener" className="font-mono text-[.5rem] tracking-[.04em] uppercase text-clay underline underline-offset-2 mt-0.5">reference ↗</a>
      )}
    </div>
  );
}
