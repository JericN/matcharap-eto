...
if (!url || !token) {
  ...
  console.error('  Set UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN (or the KV_* pair) in web/.env.local,');
...
try {
  ...
  console.log('   Current shared `state` key:', state ? JSON.stringify(state) : '(empty → defaults to {srp:{},saved:[]})');
  process.exit(0);
} catch (e) {
